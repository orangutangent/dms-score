...
interface ScaleServiceTemplateInputProps {
  question: QuestionType;
  initialAnswer: Answer;
  onAnswerChange: (answer: Answer) => void;
}

const ScaleServiceTemplateInput: React.FC<ScaleServiceTemplateInputProps> = ({
  question,
  initialAnswer,
  onAnswerChange,
}) => {
            ...
            <div className="space-y-3">
              {question.options?.map((option) => (
                <Radio
                  ...
                  name={`scale-service-${subQ.service}`}
                />
              ))}
            </div>
          </div>
...
export default ScaleServiceTemplateInput;
