...
export async function GET() {
  try {
    const digitalMaturityResults = await prisma.digitalMaturitySurveyResult.findMany();
    ...
    digitalMaturityResults.forEach(result => {
      ...
      if (!statsByCountry[country]) {
        statsByCountry[country] = {
          digitalMaturity: { count: 0, totalScore: 0, criterion: {}, average: 0 },
      ...
      for (const [criterion, score] of Object.entries(result.criterionScores as Record<string, number>)) {
        if (!statsByCountry[country].digitalMaturity.criterion[criterion]) {
          statsByCountry[country].digitalMaturity.criterion[criterion] = { total: 0, count: 0, average: 0 };
        }
        statsByCountry[country].digitalMaturity.criterion[criterion].total += score;
    ...
    governmentResults.forEach(result => {
      ...
      for (const [criterion, score] of Object.entries(result.criterionScores as Record<string, number>)) {
        if (!statsByCountry[country].government.criterion[criterion]) {
          statsByCountry[country].government.criterion[criterion] = { total: 0, count: 0, average: 0 };
        }
    ...
    for (const country in statsByCountry) {
      ...
      if (countryStats.digitalMaturity.count > 0) {
        countryStats.digitalMaturity.average = countryStats.digitalMaturity.totalScore / countryStats.digitalMaturity.count;
      ...
      if (countryStats.government.count > 0) {
        countryStats.government.average = countryStats.government.totalScore / countryStats.government.count;
